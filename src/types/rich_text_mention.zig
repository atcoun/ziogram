...
type: enums.RichTextType = .mention,
...
pub fn jsonParseFromValue(allocator: std.mem.Allocator, value: std.json.Value, options: std.json.ParseOptions) !@This() {
    return .{
        ...
        .username = try std.json.innerParseFromValue([]const u8, allocator, value.object.get("username") orelse return error.MissingField, options),
    };
}
